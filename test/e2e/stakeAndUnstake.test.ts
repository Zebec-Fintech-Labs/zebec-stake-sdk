	...
	describe("stake() => unstake()", () => {
		it("transfer token to lockup for staking", async () => {
			...
			const timestamps = new Set(
				Array.from({ length: timeB - timeA + 1 }, (_, i) =>
					(timeA + i).toString(),
				),
			);
			await sleep(3000);
			...
			assert(
				info,
				`Stake info does not exits for stake address: ${stake.toString()}`,
			);
			assert.strictEqual(
				info.address,
				stake.toString(),
				"Stake address does not match",
			);
			console.log(
				"CreatedTime:",
				new Date(info.createdTime * 1000).toUTCString(),
			);
			assert(
				timestamps.has(info.createdTime.toString()),
				"Stake created date does not fall within TimeA and TimeB",
			);
			assert.strictEqual(
				info.lockup,
				lockup.toString(),
				"Lockup address of stake does not match",
			);
			assert.strictEqual(
				info.lockPeriod,
				lockPeriod,
				"Lock period does not match",
			);
			assert.strictEqual(
				info.nonce.toString(),
				nonce.toString(),
				"Nonce of stake does not match",
			);
			assert.strictEqual(
				info.rewardAmount,
				"0",
				"Reward amount should be zero",
			);
			assert(!info.stakeClaimed, "Stake should not have been claimed");
			assert.strictEqual(
				info.staker,
				wallet.publicKey.toString(),
				"Staker does not match",
			);
		});
		...
		it("transfer token to lockup for staking", async () => {
			...
			assert(
				lockupInfo,
				`LockupInfo does not exists for lockup: ${lockup.toString()}`,
			);
			const rewardTokenDecimals = await getMintDecimals(
				connection,
				new PublicKey(lockupInfo.rewardToken.tokenAddress),
			);
			...
			assert.strictEqual(
				info.rewardAmount,
				calculatedReward,
				"Reward amount does not match calculated amount",
			);
		});
	});
});
