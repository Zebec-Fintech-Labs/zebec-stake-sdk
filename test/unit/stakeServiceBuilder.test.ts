...
describe("Stake service builder test", () => {
	it("should be created with default values", () => {
		try {
			...
			assert(
				service instanceof StakeService,
				"Service is not instance of StakeService",
			);
		} catch (error: any) {
	...
	it("should create stake service with anchor provider", () => {
		try {
			const service = new StakeServiceBuilder()
				.setNetwork(network)
				.setProvider(
					createAnchorProvider(connection, wallet, { commitment: "confirmed" }),
				)
				.setProgram((provider) => new Program(ZEBEC_STAKE_IDL_V1, provider))
				.build();
	...
	it("should throw error when network is set twice", () => {
		try {
			...
			assert.strictEqual(
				error.message,
				"InvalidOperation: Network is set twice.",
			);
		}
	});
	...
	it("should throw error when provider is set twice", () => {
		try {
			...
			assert.strictEqual(
				error.message,
				"InvalidOperation: Provider is set twice.",
			);
		}
	});
	...
	it("should throw error when program is set twice", () => {
		try {
			...
			assert.strictEqual(
				error.message,
				"InvalidOperation: Program is set twice.",
			);
		}
	});
});
